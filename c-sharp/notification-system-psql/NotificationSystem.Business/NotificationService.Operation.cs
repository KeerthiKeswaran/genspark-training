    ...
    public partial class NotificationService
    {
        ...
        public (string Status, User? ExistingUser) AuthenticateUser(User user, bool isRegistering)
        {
            ...
            if (isRegistering)
            {
                if (userByEmail != null)
                {
                    Console.WriteLine($"Registration Failed: User already exists with email {user.Email}.");
                    return ("EmailExists", null);
                }
                
                if (userByPhone != null)
                {
                    Console.WriteLine($"Registration Failed: User already exists with phone number {user.PhoneNumber}.");
            ...
            else // Login Logic
            {
                ...
                {
                    throw new NotificationException($"Login Failed: User registered with email {user.Email} but a different phone number.");
                }

                if (userByPhone != null && userByPhone.Name.Equals(user.Name, StringComparison.OrdinalIgnoreCase) && userByPhone.Email != user.Email)
                {
                    throw new NotificationException($"Login Failed: User registered with phone {user.PhoneNumber} but a different email.");
                }
