        ...
        public void ProcessNotification(string type, User sender, string receiverName, string contact, string message)
        {
            try
            {
                ...
                Notification sentNotif = (type == "Email") 
                    ? new EmailNotification(GenerateId(Guid.NewGuid().ToString()), message, DateTime.Now, receiver, sender, "Sent")
                    : new SmsNotification(GenerateId(Guid.NewGuid().ToString()), message, DateTime.Now, receiver, sender, "Sent");

                // Save for Sender
                AddNotification(sender.Id, sentNotif);

                // Save for Receiver (Mirror Copy)
                Notification receivedNotif = CloneWithStatus(sentNotif, "Received");
                AddNotification(receiver.Id, receivedNotif);
